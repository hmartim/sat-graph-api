...
| **Structural Navigation** | Hierarchy traversal and context retrieval | Fully deterministic | `getItemAncestors`, `getItemHierarchy` |
...
| `getValidVersion` | Item ID + timestamp | Version object | Get the version valid on a specific date |
...
| Action | Input | Output | Use Case |
|--------|-------|--------|----------|
| `getValidVersion` | Item ID + timestamp | Version object | "What was Article 6 on Jan 1, 2000?" |
| `getVersionsInInterval` | List of Item IDs + start date + end date | List of Versions | "All versions of items in chapter X from 2010-2020" |

**Design Note:** The endpoint deterministically retrieves the last version valid at any point during the day of the given timestamp.
